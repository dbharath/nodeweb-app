...
(function (exports) {
  ...
  var utils = {
    ...
    validatePincode: function (code) {
      code = this.stringTrim('' + code);
      return code.length === 6 && (/^[1-9][0-9]{5}$/).test(code);
    },
    ...
    validateCityName   : function(city){
      city = this.stringTrim('' + city);
      return city && (/^[a-zA-Z0-9.,'\-\s()]+$/).test(city);
    },
    ...
    // Reduce a string length to a specific number 
    // Any trimming will be marck with a specific clamp info string  
    clampString: function (str, limit) { 
      var clampedStr = str;  
          limit     *= 1;
       
      // Check if all income variables match logic criteria  
      if (typeof str == 'string' && !isNaN(limit) && str.length > limit) { 
        var clampInfoStr = '...',  
        clampInfoStrLength = clampInfoStr.length;  
         
        // Check if the string limiter allow clamping message and clamp info at ⬅
        if (limit > clampInfoStrLength) {  
          limit -= clampInfoStrLength; 
          clampedStr = str.substr(0, limit) + clampInfoStr;  
           
        } else { 
         
          clampedStr = clampInfoStr.substr(0, limit);  
        }  
      }  
    ...
    getUserURL: function (str) {
      return this.stringTrimAndToLower(str).replace(/ /g, '-').replace(/[-]+/, '-');
    }
  };
